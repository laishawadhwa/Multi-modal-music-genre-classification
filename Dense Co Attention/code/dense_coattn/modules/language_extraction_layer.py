...
class MTLSTM(nn.Module):
	...
	def forward(self, inputs, mask):
		...
		h0 = self.init_hidden(torch.tensor(torch.arange(0, 2*2)).type_as(inputs)\
					.unsqueeze(1).long()).expand(2*2, batch, 300).contiguous()
		c0 = self.init_hidden(torch.tensor(torch.arange(2*2, 2*2*2)).type_as(inputs)\
					.unsqueeze(1).long()).expand(2*2, batch, 300).contiguous()
...
class LSTM(nn.Module):
	...
	def forward(self, inputs, mask):
		...
		lens = lens.data.long().tolist()
		self.rnn.flatten_parameters()
		h0 = self.init_hidden(torch.tensor(torch.arange(0, 2*self.num_layers)).type_as(inputs)\
					.unsqueeze(1).long()).expand(2*self.num_layers, batch, self.rnn_dim).contiguous()
		c0 = self.init_hidden(torch.tensor(torch.arange(2*self.num_layers, 2*2*self.num_layers)).type_as(inputs)\
					.unsqueeze(1).long()).expand(2*self.num_layers, batch, self.rnn_dim).contiguous()
